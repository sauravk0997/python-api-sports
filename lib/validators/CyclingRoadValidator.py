...
from lib.schemas.CyclingRoadSportSchema import *
...
class CyclingRoadValidator(object):
    """JSON validation for ESPN Sports Core API"""

    def __init__(self, *p, **k):
        pass

    @keyword('Sport Schema from ${response} should be valid for cycling road', tags=['schema checks', 'functional', 'CoreV3'], types={'response': requests.Response})
    def sports_resp_is_valid_for_cycling_road(self, response) -> bool:
        ...
        """
        try:
            schema = CyclingRoadSportSchema().load(response.json())     
