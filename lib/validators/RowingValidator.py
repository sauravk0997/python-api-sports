...
from lib.schemas.RowingSportSchema import RowingSportSchema
from lib.validators.ESPNSportsCoreCommonValidator import *


@library(scope='GLOBAL', version='5.0.2')
class RowingValidator(object):
    """
    ...
    @keyword('Sport Schema from ${response} should be valid for rowing', tags=['schema checks', 'functional', 'CoreV3'],
             types={'response': requests.Response})
    def sports_resp_is_valid_for_rowing(self, response) -> bool:
        ...
        """
        try:
            schema = RowingSportSchema().load(response.json())
