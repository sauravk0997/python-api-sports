...
class CrossCountrySportsCoreValidator(object):
    """JSON validation for ESPN Sports Core API"""

    def __init__(self, *p, **k):
        pass

    @keyword('Sport Schema from ${response} should be valid for Cross-Country',
             tags=['schema checks', 'functional', 'CoreV3'], types={'response': requests.Response})
    def sports_resp_is_valid(self, response) -> bool:
        """ Schema for the endpoint: /v3/sports/cross-country
        ...
        """
        try:
            schema = CrossCountrySportSchema().load(response.json())
