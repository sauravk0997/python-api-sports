...
from lib.schemas.LugeSportSchema import *
...
class LugeValidator(object):
    """JSON validation for ESPN Sports Core API"""

    def __init__(self, *p, **k):
        pass

    @keyword('Sport Schema from ${response} should be valid for luge', tags=['schema checks', 'functional', 'CoreV3'], types={'response': requests.Response})
    def sports_resp_is_valid_for_cycling_bmx(self, response) -> bool:
        ...
        """
        try:
            schema = LugeSportSchema().load(response.json())     
