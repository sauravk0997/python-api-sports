...
class EquestrianLeagueSchema(LeagueSchema):
    class Meta:
        unknown = RAISE
...
class EquestrianSportSchema(SportSchema):
    class Meta:
        unknown = RAISE

    leagues = fields.List(fields.Nested(EquestrianLeagueSchema), required=False)


if __name__ == '__main__':
    target = 'https://sports.core.api.espn.com/v3/sports/equestrian?enable=leagues'
    resp = requests.get(target)

    if resp.status_code == 200:
        try:
            page = EquestrianLeagueSchema().load(resp.json())
