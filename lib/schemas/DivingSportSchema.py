...
class DivingLeagueSchema(LeagueSchema):
    class Meta:
        unknown = RAISE
...
class DivingSportSchema(SportSchema):
    class Meta:
        unknown = RAISE

    leagues = fields.List(fields.Nested(DivingLeagueSchema), required=False)


if __name__ == '__main__':
    target = 'https://sports.core.api.espn.com/v3/sports/Diving?enable=leagues'
    resp = requests.get(target)

    if resp.status_code == 200:
        try:
            page = DivingLeagueSchema().load(resp.json())
