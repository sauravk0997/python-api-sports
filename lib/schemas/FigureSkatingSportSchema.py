...
class FigureSkatingLeagueSchema(LeagueSchema):
    class Meta:
        unknown = RAISE
...
class FigureSkatingSportSchema(SportSchema):
    class Meta:
        unknown = RAISE

    leagues = fields.List(fields.Nested(FigureSkatingLeagueSchema), required=False)


if __name__ == '__main__':
    target = 'https://sports.core.api.espn.com/v3/sports/figure-skating?enable=leagues'
    resp = requests.get(target)

    if resp.status_code == 200:
        try:
            page = FigureSkatingLeagueSchema().load(resp.json())
