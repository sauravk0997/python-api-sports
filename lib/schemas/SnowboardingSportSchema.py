...
class SnowboardingLeagueSchema(LeagueSchema):
    class Meta:
        unknown = RAISE
...
class SnowboardingSportSchema(SportSchema):
    class Meta:
        unknown = RAISE

    leagues = fields.List(fields.Nested(SnowboardingLeagueSchema), required=False)


if __name__ == '__main__':
    target = 'https://sports.core.api.espn.com/v3/sports/snowboarding?enable=leagues'
    resp = requests.get(target)

    if resp.status_code == 200:
        try:
            page = SnowboardingLeagueSchema().load(resp.json())
