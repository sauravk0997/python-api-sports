...
class SkateboardingLeagueSchema(LeagueSchema):
    class Meta:
        unknown = RAISE
...
class SkateboardingSportSchema(SportSchema):
    class Meta:
        unknown = RAISE

    leagues = fields.List(fields.Nested(SkateboardingLeagueSchema), required=False)


if __name__ == '__main__':
    target = 'https://sports.core.api.espn.com/v3/sports/skateboarding?enable=leagues'
    resp = requests.get(target)

    if resp.status_code == 200:
        try:
            page = SkateboardingLeagueSchema().load(resp.json())
